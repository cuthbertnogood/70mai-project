...
| `stage_batch_clips` | `10` | Сколько клипов копировать на SSD за волну; каждый batch → независимый part, затем один final concat. После copy — проверка hash+ffprobe (битые USB-копии перекачиваются). Concat без `-probesize 1M` (`moov` в конце у 70mai). При fail parts сохраняются; после **3× merge short** — `[i]/[r]` | **каждый merge** |
...
For unattended runs with the lid closed, `./scripts/autopilot.sh` enables the same `pmset disablesleep` + `caffeinate` for the whole session (`AUTOPILOT_AWAKE=1` by default; skipped for `--dashboard-only`). The legacy **watchdog** does the same via `WATCH_AWAKE=1` (see [Upload watchdog](#autopilot-sd-card--youtube-zero-manual-steps)).
...
**Stale publish state (same `card_id`, different footage):** skip/upload decisions use `wall_start` + `trip_indices` (and `duration_sec` for Event/Parking), not chunk index alone. If SD `publish_*.state.json` marks chunk 1 uploaded for `2026-07-06` but the current plan’s chunk 1 starts `2026-07-29`, autopilot logs `Publish state mismatch` / `Stale publish state: dropped …`, surgically removes those rows, and re-queues the chunks. Copying `.70mai/` between cards without a new `card_id.txt` is the usual cause.
...
**Auto-repair (Parking/Event):** before each chunk, `pipeline_repair.py` checks SSD merges cover the planned trip (≥98% with ffprobe). Codes are written to `video/Output/.publish_tmp/repair_log.jsonl` and shown in the dashboard footer **── Сбои ──** (with recent ERROR lines from `publish_all.log`).
...
Reads `autopilot_status.json`, publish state on SD, `chunk_*/trip_*.mp4` sizes, and the upload diagnostic JSONL — no subprocess coupling. Screen layout lives in **`lib/autopilot_dashboard_view.py`** (hot-reloaded on save); the parent process also respawns the worker when that file or `autopilot_dashboard.py` changes — no manual restart. Header lists each pipeline step on its own line (`copy` / `merge` / `compose` / `upload`) with status **`► активно`**, **`✓ готово`**, or **`· ждёт`**, then **proc/status** and **disk/YT**. Footer **── Сбои ──** shows recent `repair_log.jsonl` diagnoses plus ERROR/retry lines from `publish_all.log`, each prefixed with local date/time. Stale `status.json` (>5 min) suppresses ghost «► compose»; if an **import** process is still alive, progress is taken from **`publish_all.log`**. Autopilot passes **`--status-dir`** into import so heartbeats update `autopilot_status.json`. On short terminals a long run of ✓ trips is collapsed (`✓ 1–16/19 готово`). Flags: `--source`, `--wait`, `--types`, `--temp-dir`, `--interval` (default 1s).
...
Master log: `video/Output/.publish_tmp/publish_all.log`. Autopilot mirrors **all** its own messages (plan, step headers, done summary) into this file, not only subprocess output. Import merge logs a **heartbeat every 30s** while ffmpeg concat runs (`… merging FILE (elapsed)`); the live dashboard mirrors that as **`импорт N%`**. Atomic mkdir lock (`.publish_all.lock/` + `pid`) prevents duplicate autopilot runs.
...
| Lock dir | Atomic mkdir `.publish_all.lock/` (pid inside); refuses a second instance unless `--force-restart` |
