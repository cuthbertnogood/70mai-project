...
**I/O acceleration (automatic):**

- **One merge worker by default** (`merge_workers: 1`) — parallel concat on USB/SD causes seek thrashing.
- **Local staging** — SD → SSD batches → `ffmpeg concat -c copy` → delete staged clips.
- **Page-cache prefetch** — only when `merge_workers == 1` and `prefetch: true`.
- **ffprobe cache** — `video/Output/.probe_cache.json`.
- **Fast concat start** — `-probesize 1M -analyzeduration 0`.

### Runtime config (`70mai_runtime.json`)

Файл в корне проекта: [`70mai_runtime.json`](70mai_runtime.json).  
Опциональный override (приоритетнее, удобно править во время прогона):  
`video/Output/.publish_tmp/70mai_runtime.json`.

В логе при смене значений:
...
| Метка | Смысл |
|-------|--------|
| **каждый merge** | до следующего файла склейки (не ждать конец import) |
| **группа камеры** | со следующей группы (`Normal/Front` → `Normal/Back` …) |
| **шаг publish** | перед следующим `publish_70mai` (тип / кусок) |
| **новый import** | только после перезапуска `import_70mai` / retry с новым cmd / нового автопилота |
| **пока не используется** | ключ в JSON есть, код ещё не читает |
...
| `chunk_minutes` | `10` | Макс. длительность чанка (мин); вместе с `chunk_clips` — кто раньше сработает | **новый import** |
| `stage_batch_clips` | `10` | Сколько клипов копировать на SSD за одну волну; если меньше `chunk_clips` — несколько stage→concat→delete | **каждый merge** |
| `gap_seconds` | `120` | Новый session, если пауза между клипами больше N сек | **новый import** |
| `merge_workers` | `1` | Параллельных ffmpeg concat на одну камеру; на USB держите `1` | **новый import** |
| `prefetch` | `true` | Прогрев page cache следующего чанка (только при `merge_workers=1`) | **каждый merge** (цикл prefetch) / **группа** (старт) |
| `prefetch_batches` | `2` | Сколько чанков prefetch держит «впереди» | **группа камеры** |
| `probe_workers` | `8` | Параллельных ffprobe при замере длительностей | **группа камеры** |
| `merge_heartbeat_sec` | `30` | Интервал строк `… merging` в логе | **пока не используется*** |
| `merge_max_attempts` | `3` | Повторы ffmpeg на один выходной файл | **каждый merge** |
| `merge_retry_delay_sec` | `3` | Пауза между повторами ffmpeg | **каждый merge** |

\* `merge_heartbeat_sec` задан в JSON/DEFAULTS; heartbeat сейчас берётся из константы `MERGE_HEARTBEAT_SEC` в коде — смена в JSON пока не действует (нужен небольшой follow-up).

CLI-эквиваленты (стартовые значения из JSON): `--chunk-clips`, `--chunk-minutes`, `--stage-batch-clips`, `--merge-workers`, `--gap-seconds`.

#### `autopilot.*`

| Параметр | Default | Что делает | На лету |
|----------|---------|------------|---------|
| `publish_chunk_minutes` | `120` | Целевой размер trip-chunk для YouTube (минуты) | **шаг publish** |
| `session_gap` | `120` | Gap сессий для plan/publish (сек) | **новый import** / старт автопилота (план уже посчитан) |
| `import_merge_retry_max` | `3` | Сколько раз автопилот перезапускает весь import при fail | при **старте** блока import (до цикла retry) |
| `import_merge_retry_delay_sec` | `15` | Пауза между полными retry import | при **старте** блока import |
| `min_free_gb` | `20` | Мин. свободное место на диске перед compose | **шаг publish** |
| `profile` | `balanced` | Профиль compose (`balanced` / `draft` / `quality` / `hevc`) | **шаг publish** |
| `prune_merged` | `after-compose` | Когда удалять merged: `off` / `after-compose` / `after-upload` | **шаг publish** |
| `sd_poll_sec` | `15` | Интервал опроса SD в `--wait` | **пока не используется** (в коде ещё `SD_POLL_SEC`) |

**Практика тюнинга USB:** чаще всего крутят `stage_batch_clips` и `chunk_clips`. Первое меняется сразу (следующий файл); второе — со следующей камеры или после рестарта import.
