...
flowchart TB
  subgraph import_phase [Фаза Import]
    J1["каждый merge:<br/>stage_batch_clips<br/>merge_max_attempts<br/>merge_retry_delay_sec<br/>prefetch"]
    J2["группа камеры:<br/>chunk_clips<br/>probe_workers<br/>prefetch_batches"]
...
| `prefetch` | `true` | Прогрев page cache следующего чанка (только при workers=1) | **каждый merge** / группа |
...
Связанное ускорение I/O: один merge-worker по умолчанию, staging SD→SSD, prefetch при workers=1, кэш ffprobe, быстрый старт concat (`-probesize 1M`).
