...
| `stage_batch_clips` | `10` | Сколько клипов копировать на SSD за волну; каждый batch → независимый part, затем один final concat (без growing `partial⊕next`). При fail хорошие `_part_*` сохраняются (resume без SD). После **3× merge short** — стоп и выбор `[i] ignore` / `[r] retry` | **каждый merge** |
