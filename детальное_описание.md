...
- **One merge worker by default** (`--merge-workers 1`) — parallel concat on a USB/SD reader causes seek thrashing and lower aggregate throughput. Raise only if the source is a fast local disk.
- **Local staging** — for each ~10 min chunk, minute clips are copied from the SD to `…/.merge_stage/<output_stem>/`, then `ffmpeg concat -c copy` runs from the local copies; staged clips are deleted right after a successful (or failed) merge.
- **Page-cache prefetch** — enabled only when `--merge-workers 1`: while the current chunk is concat’d locally, a background thread warms the next pending chunk from the SD.
...
Import flag: `--merge-workers N` (default `1`).
