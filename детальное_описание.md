...
| `stage_batch_clips` | `10` | Сколько клипов копировать на SSD за волну; каждый batch → независимый part, затем один final concat (без growing `partial⊕next`, из‑за которого Front 4K терял ~7 мин) | **каждый merge** |
