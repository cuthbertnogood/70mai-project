...
Reads `autopilot_status.json`, publish state on SD, `chunk_*/trip_*.mp4` sizes, and the upload diagnostic JSONL — no subprocess coupling. Header lists each pipeline step on its own line (`copy` / `merge` / `compose` / `upload`) with status **`► активно`**, **`✓ готово`**, or **`· ждёт`**, then **proc/status** and **disk/YT**. Stale `status.json` (>5 min) suppresses ghost «► compose»; if an **import** process is still alive, progress is taken from **`publish_all.log`** (`[copy]` / `[merge]`). Autopilot passes **`--status-dir`** into import so heartbeats update `autopilot_status.json`. On short terminals a long run of ✓ trips is collapsed (`✓ 1–16/19 готово`). Flags: `--source`, `--wait`, `--types`, `--temp-dir`, `--interval` (default 1s).
