...
- **One merge worker by default** (`merge_workers: 1` in [`70mai_runtime.json`](70mai_runtime.json) / `--merge-workers`) — parallel concat on USB/SD causes seek thrashing.
- **`chunk_clips` / `chunk_minutes`** — how many minute clips (and/or minutes) go into one merged file.
- **`stage_batch_clips`** — how many clips to copy to SSD per staging wave; if smaller than the chunk, merge runs in batch-concat passes and deletes each wave after use.
- **Hot reload** — edit `70mai_runtime.json` (or override `video/Output/.publish_tmp/70mai_runtime.json`) while import runs; values are re-read before each merge job (`stage_batch_clips`, retries, prefetch) and between camera groups / publish steps. `chunk_clips` applies from the next camera group (or next import process).
- **Page-cache prefetch** — only when `merge_workers == 1` and `prefetch: true`.
- **ffprobe cache** — durations in `video/Output/.probe_cache.json`.
- **Fast concat start** — `-probesize 1M -analyzeduration 0`.

Import flags: `--chunk-clips N`, `--stage-batch-clips N`, `--merge-workers N` (defaults from JSON).
