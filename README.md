...
**Веб-Dashboard** (`127.0.0.1:8787`): порядок сверху вниз — **кнопки управления** (Stop / Skip / …), **два конвейера** (**«Сейчас»** + **«Вся карта»**), **карта файлов** (SD + HDD; квадратики **в работе** — жёлтая рамка, обновление ~1 с), затем таблицы прогона и SD. Слева — прогон (Import / Compose / Upload); **справа — таблица SD** (поездки, длительность, место на карте). Строки таблицы SD подсвечены по состоянию импорта: зелёный — **загружено** на YouTube, голубой — **импортировано** (есть merge на SSD или ledger, пересекающийся с клипами **этой** карты; старые mega-merge с других дат выкидываются), жёлтый — **в плане** / **частично**, серый — **не импортировано**. Карта файлов: матрица блоков на флешке и merge-MP4 на SSD (счётчики скопировано/смержено/YouTube; **«В работе N · copy/merge|compose|upload»** — рамка и счётчик). Наведите на блок — имя и статус. `GET /api/filemap` (~15 с) + `processing` в `/api/status` (~1 с). После обновления кода перезапустите `./scripts/autopilot.sh` или поднимите `--dashboard-only` на другом порту (см. выше).
