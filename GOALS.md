...
Отсутствующая/короткая камера в слоте заменяется **чёрным видео + тишиной** (`color=black` / `anullsrc`), длина `plan_estimate` для Parking/Event = сумма union-слотов `max(front, back)`. После compose ffprobe сверяет итоговую длительность с целевой; **после upload** — длительность на YouTube (`contentDetails.duration`) vs локальный mp4 (≥98%, иначе upload failed, state не обновляется). `--repair` требует свежий manifest (иначе `manifest_missing` → rebuild); при наличии manifest coverage-gap’ы — предупреждение с black-fill, а не blocker/cap. Legacy-путь (без manifest) сохранён для старых merge. Логика: `lib/clip_timeline.py`.
